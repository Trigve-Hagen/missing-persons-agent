### Missing Persons Agent
Define and create models that do tasks related to finding missing people. 
  
### Models
Predict the area based upon distance able to travel in a time frame. Start from estimated time of found missing. Figure out the distance they could have traveled. Send email alerts to gas stations, markets, restaraunts, hotels, hospitals and shelters. Update to larger areas as the search progresses.  
Create an API endpoint that can load email lists to send the alerts to.
   
Search for people who have been missing for a long time in different countries. USA, Mexico, Canada etc..  
Look up all places that have public records about people to search through.
